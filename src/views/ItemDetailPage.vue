...
const isCustomizable = computed(() => itemModel.value?.isCustomizable() || false);
...
// 使用颜色显示
const { conicGradientStyle } = useColorDisplay(displayColors);

// 生成变体颜色样式
const getVariantColorStyle = (colors: string[]) => {
        ...
        <template v-else>
            ...
            <div class="detail-content">
                ...
                <div class="info-section">
                    ...
                    <div class="info-grid">
                        ...
                        <div v-if="displayColors.length > 0" class="info-item">
                            ...
                            <div class="color-display">
                                ...
                                <span class="color-block" :style="{ background: conicGradientStyle }"></span>
                            </div>
                        </div>

                        <div v-if="sources.length > 0" class="info-item full-width">
                            <label>来源:</label>
                            <span>📍 {{sources.map(s => getSourceName(s)).join(', ')}}</span>
                        </div>
                        ...
                        <div v-if="exchangePrice && exchangeCurrency" class="info-item">
                            <label>兑换价格:</label>
                            <span class="price">🎫 {{ formatPrice(exchangePrice) }} {{ exchangeCurrency }}</span>
                        </div>
                    </div>

                    <!-- 功能特性 -->
                    <div v-if="isDIY || isCustomizable || isOutdoor || isInteractive || surface !== undefined"
                    ...
                    <!-- 其他属性 -->
                    <div v-if="hhaPoints || hhaCategory || seasonEvent || stackSize > 1 || speakerType || lightingType || foodPower"
                        ...
                        <div class="info-grid">
                            <div v-if="concepts.length > 0" class="info-item full-width">
                                ...
                                <div class="concepts-list">
                                    <span v-for="(concept, idx) in concepts" :key="idx" class="concept-tag">
                                        {{ concept }}
                                    </span>
                                </div>
                            </div>
            ...
            <div v-if="allVariants.length > 0" class="variants-section">
                ...
                <div class="variants-grid">
                    <div v-for="(variant, idx) in allVariants" :key="idx" class="variant-card">
                        <div class="variant-image">
                            <img :src="variant.imageUrl" :alt="`${variant.variantName} - ${variant.patternName}`" />
                        </div>
                        <div class="variant-info">
                            <div class="variant-id-row">
                                <span class="variant-id-text">ID: {{ variant.id }}</span>
                                <span v-if="variant.colors.length > 0" class="variant-color-circle"
                                    :style="{ background: getVariantColorStyle(variant.colors) }"></span>
                            </div>
